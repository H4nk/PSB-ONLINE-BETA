...
if(isset($_POST['pencet'])){
    ...
    $new=md5($ulangi);
    $k=mysql_fetch_array(mysql_query("select kunci from kepsek where nama='$panitia'"));
    if($lama == $k['kunci']){
        if($baru != '' && $ulangi != ''){       
            if($baru == $ulangi){
             $input=mysql_query("update kepsek set kunci='$new' where nama='$panitia'");
             if($input){
...
<table style='font-size: 12px; '>
    <tr>
        <td colspan='3'><img src='../img/icon/kunci.png'> Ganti Password Kepala Sekolah<hr></td>
    </tr>
    <tr>
